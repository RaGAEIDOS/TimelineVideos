...
| 🔄 **استئناف التشغيل** | يتذكر آخر فيديو كنت تشاهده ويستأنف من نفس النقطة تلقائياً |
...
```powershell
# نسخ مكتبات Qt الضرورية
windeployqt build\TimelineVideo.exe

# نسخ مكتبات VLC (عدّل المسار حسب تثبيت VLC لديك)
cp "C:/Program Files/VideoLAN/VLC/libvlc.dll" build/
cp "C:/Program Files/VideoLAN/VLC/libvlccore.dll" build/

# تشغيل
.\build\TimelineVideo.exe
```
...
عند إغلاق البرنامج أثناء مشاهدة فيديو، يتذكر التطبيق:
- **آخر فيديو كنت تشاهده** — يتم تحديد البلاي ليست والفيديو تلقائياً عند فتح البرنامج
- **نقطة التوقف** — عند الضغط على **Space** أو النقر المزدوج، يستأنف التشغيل من نفس النقطة التي توقفت عندها

تظهر رسالة في شريط الحالة: "آخر مشاهدة: [اسم الفيديو] - اضغط Space للاستئناف"
